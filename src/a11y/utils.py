...
def _serialize_crew_output(crew_output: Any) -> dict:
...
async def _save_detailed_results(results: Dict[str, Any], results_path: Path) -> None:
    """Save detailed analysis results"""
    try:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        results_dir = results_path / timestamp
...
async def process_results(crew_output: Any, context: Dict[str, Any]) -> Dict[str, Any]:
    """Process the test results with WCAG integration"""
    try:
        # Extract results from crew output
        results = _serialize_crew_output(crew_output)
        ...
        if choice == 'q':
            ...
            try:
                ...
                if 1 <= file_choice <= len(html_files):
                    ...
                    url, server = _create_file_server(selected_file)
